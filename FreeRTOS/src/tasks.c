...
typedef struct tskTaskControlBlock
{
    ...
    TickType_t          runTime;
    int                 index;
    UBaseType_t         nominalPriority;
    UBaseType_t         maxPriority;
...
StackType_t *pxTopOfStack;
	...
	if( pxNewTCB != NULL )
	{
	    pxNewTCB->index = cnt;
	    pxNewTCB->runTime = 0;
	    pxNewTCB->nominalPriority = taskArray[cnt].priority;
	    nizTCB[cnt] = pxNewTCB;
	    pxNewTCB->blocked = 0;
	    pxNewTCB->resourcesHeld = 0;
	    cnt++;
		#if( portUSING_MPU_WRAPPERS == 1 )
...
void intern_wait(Sem *sem){
    if (sem->locked == 0){
        sem->locked = 1;
        sem->holderIndex = pxCurrentTCB->index;
        pxCurrentTCB->resourcesHeld++;
    } else {
        TCB_t *blockedTCB;
        blockedTCB = nizTCB[sem->holderIndex];
        if( listIS_CONTAINED_WITHIN( &( pxReadyTasksLists[ blockedTCB->uxPriority ] ), &( blockedTCB->xGenericListItem ) ) != pdFALSE )
				{
					if( uxListRemove( &( blockedTCB->xGenericListItem ) ) == ( UBaseType_t ) 0 )
					{
						taskRESET_READY_PRIORITY( blockedTCB->uxPriority );
					}
					else
					{
						mtCOVERAGE_TEST_MARKER();
                    ...
                    blockedTCB->uxPriority = pxCurrentTCB->uxPriority;

					prvAddTaskToReadyList(blockedTCB);

				}
        blockedTCB->uxPriority = pxCurrentTCB->uxPriority;
        sem->blockQueue[sem->queueCount++] = pxCurrentTCB->index;

                    if (pxCurrentTCB->blocked == 0)
                        pxCurrentTCB->lock = sem;
					pxCurrentTCB->blocked++;

					if (blockedTCB->blocked != 0){
                        intern_wait(blockedTCB->lock);
					}
    }
}
...
void pi_signal(Sem *sem){
    ...
    UBaseType_t uxPriorityUsedOnEntry = pxCurrentTCB->uxPriority;
    pxCurrentTCB->resourcesHeld--;
    if (pxCurrentTCB->resourcesHeld == 0){
        pxCurrentTCB->uxPriority = pxCurrentTCB->nominalPriority;

    if( ( listGET_LIST_ITEM_VALUE( &( pxCurrentTCB->xEventListItem ) ) & taskEVENT_LIST_ITEM_VALUE_IN_USE ) == 0UL )
				{
					listSET_LIST_ITEM_VALUE( &( pxCurrentTCB->xEventListItem ), ( ( TickType_t ) configMAX_PRIORITIES - ( TickType_t ) 0 ) );
				}
				else
				{
					mtCOVERAGE_TEST_MARKER();
				...
				if( listIS_CONTAINED_WITHIN( &( pxReadyTasksLists[ uxPriorityUsedOnEntry ] ), &( pxCurrentTCB->xGenericListItem ) ) != pdFALSE )
				{
    ...
    TCB_t *res = nizTCB[sem->blockQueue[0]];
        int k = 0;
        for (int i = 1; i < sem->queueCount; i++){
        TCB_t *temp = nizTCB[sem->blockQueue[i]];
            if (temp->uxPriority > res->uxPriority){
                res = temp;
                k = i;
            }
        }

        while (k+1<sem->queueCount){
            sem->blockQueue[k] = sem->blockQueue[k+1];
            k++;
        }
        ...
        res->blocked--;
        sem->holderIndex = res->index;
        if (res->blocked == 0){
            res->resourcesHeld++;
            testResume(res);
        }

        if (sem->queueCount == 0 && sem != res->lock)
            sem->locked = 0;
    }
}
...
int startTask(int ind){
    TCB_t *pxTCB;
    UBaseType_t myPriority = 1;
    for (int i = 0; i < listCURRENT_LIST_LENGTH(&(pxReadyTasksLists[myPriority])); i++){
        listGET_OWNER_OF_NEXT_ENTRY(pxTCB, &(pxReadyTasksLists[myPriority]));
        if (pxTCB->index == ind)
            break;
    }
    if( listIS_CONTAINED_WITHIN( &( pxReadyTasksLists[ pxTCB->uxPriority ] ), &( pxTCB->xGenericListItem ) ) != pdFALSE )
				{
					...
					pxTCB->uxPriority = pxTCB->nominalPriority;
					prvAddTaskToReadyList( pxTCB );
					return 1;
				}
    return 0;
...
TickType_t getCurrentTaskRunTime(){
    return pxCurrentTCB->runTime;
}

void createMyTask(Task *task, TaskFunction_t fun, int stackDepth){
    memcpy(&taskArray[taskCounter++], task, sizeof(Task));
    if (taskCounter == 1)
        xTaskCreate(fun, "t1", stackDepth, &taskArray[taskCounter-1], 2, NULL);
    else
        xTaskCreate(fun, "t1", stackDepth, &taskArray[taskCounter-1], 1, NULL);
}
